...
- **本檔「一句話現況」的 Phase 3 敘述過時（2026-08-17 順帶發現，刻意未修）**：
  第 23 行仍寫「目前在 Phase 3（拓寬市場：台股籌碼資料）進行中」，但同段
  後續 ★ 條目已記錄台股籌碼特徵 2026-07-12 結案、台股 E2 化與放空各輪
  2026-07-16 交付；`AGENTS.md` Gate 表與 `MASTER_PLAN.md` §1.2 亦已於
  2026-08-17 對齊為「P3 ✅ 核心完成（美股未起步）」。屬「一句話現況」
  逐輪追加流水稿的歷史殘留，**不影響任何數字、參數或程式行為**。
  另開一輪處理（使用者 2026-08-17 決定不塞進 MASTER_PLAN 那輪 PR）；
  修正時建議整段重讀一次，該段是累積式追加，殘留可能不只這一處。
